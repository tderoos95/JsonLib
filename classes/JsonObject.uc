...
private function string EscapeCharacters(string Value)
{
	...
	for(i = 0; i < TextLength; i++)
	{
		...
		if(i > RunStart)
			Result $= Mid(Value, RunStart, i - RunStart);

		Result $= EscapeSequence(Code);
		RunStart = i + 1;
	}
	...
	if(TextLength > RunStart)
		Result $= Mid(Value, RunStart, TextLength - RunStart);

	return Result;
}

private function string EscapeSequence(int Code)
{
	switch(Code)
	{
		case 34: return EscapeCharacter $ QuotationMarkCharacter;
		case 92: return EscapeCharacter $ EscapeCharacter;
		case 8:  return EscapeCharacter $ "b";
		case 9:  return EscapeCharacter $ "t";
		case 10: return EscapeCharacter $ "n";
		case 12: return EscapeCharacter $ "f";
		case 13: return EscapeCharacter $ "r";
	}

	return EscapeCharacter $ "u" $ ToHex4(Code);
}

private function string ToHex4(int Code)
{
